...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_ticket'])) {
    ...
    $sql = "DELETE FROM tbl_close_regular WHERE t_aname = ?";
    ...
    $stmt->close();
    header("Location: regular_close.php");
    exit();
}
...
$sqlCount = "SELECT COUNT(*) as total FROM tbl_close_regular WHERE t_status = 'Closed'";
$resultCount = $conn->query($sqlCount);
$totalTickets = $resultCount->fetch_assoc()['total'];
$totalPages = ceil($totalTickets / $limit);

// Fetch closed regular tickets
$closedTickets = [];
$sql = "SELECT t_aname, te_technician, t_subject, t_status, t_details, t_date 
...
if ($stmt) {
    ...
    error_log("Prepare failed for closed regular tickets: " . $conn->error);
...
<head>
    ...
    <title>Admin Dashboard | Regular Tickets Record</title>
...
<div class="wrapper">
    <div class="sidebar glass-container">
        ...
        <ul>
            ...
            <li><a href="regular_close.php" class="active"><i class="fas fa-wrench"></i> <span>Regulars Record</span></a></li>
                ...
                <tbody id="ticketTable">
                    <?php if ($closedTickets): ?>
                        <?php foreach ($closedTickets as $row): ?>
                            <tr>
                                <td><?php echo htmlspecialchars($row['t_aname'] ?: '-'); ?></td>
                                <td><?php echo htmlspecialchars($row['te_technician'] ?: '-'); ?></td>
                                <td><?php echo htmlspecialchars($row['t_subject'] ?: '-'); ?></td>
                                <td><?php echo htmlspecialchars($row['t_details'] ?: '-'); ?></td>
                                <td><?php echo htmlspecialchars($row['t_date'] ?: '-'); ?></td>
                                <td class="status-closed"><?php echo htmlspecialchars($row['t_status'] ?: 'Closed'); ?></td>
                                <td class="action-buttons">
                                    ...
                                    <form method="POST" style="display:inline;" onsubmit="return confirm('Are you sure you want to delete this ticket?');">
                                        <input type="hidden" name="ticket_id" value="<?php echo htmlspecialchars($row['t_aname']); ?>">
                    ...
                    <?php else: ?>
                        <tr>
                            <td colspan="7" class="empty-state">No closed regular tickets found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
...
<div id="viewTicketModal" class="modal">
    <div class="modal-content">
        ...
        <p><strong>Ticket ID:</strong> <span id="modal-t_aname"></span></p>
        <p><strong>Technician:</strong> <span id="modal-te_technician"></span></p>
        <p><strong>Subject:</strong> <span id="modal-t_subject"></span></p>
        <p><strong>Details:</strong> <span id="modal-t_details"></span></p>
        <p><strong>Date:</strong> <span id="modal-t_date"></span></p>
        <p><strong>Status:</strong> <span id="modal-t_status"></span></p>
    </div>
</div>
...
function searchTickets(page = 1) {
    ...
    window.history.pushState({}, '', `regular_close.php?page=${page}`);
    window.location.reload(); // Reload to fetch new page data
}

function viewTicket(ticket) {
    document.getElementById('modal-t_aname').textContent = ticket.t_aname || '-';
    document.getElementById('modal-te_technician').textContent = ticket.te_technician || '-';
    document.getElementById('modal-t_subject').textContent = ticket.t_subject || '-';
    document.getElementById('modal-t_details').textContent = ticket.t_details || '-';
    document.getElementById('modal-t_date').textContent = ticket.t_date || '-';
    document.getElementById('modal-t_status').textContent = ticket.t_status || 'Closed';
