<?php
    ...
    if (isset($_POST['assign_ticket'])) {
        ...
        $sqlCheckTech = "SELECT u_username FROM tbl_user WHERE u_username = ? AND u_type = 'technician' AND u_status = 'active'";
        ...
        $stmtCheckTech->bind_result($username);
   ...
   // Handle AJAX toggle status request - SIMULATED (no database column)
if (isset($_POST['action']) && $_POST['action'] === 'toggle_status') {
    if (!isset($_POST['username']) || !isset($_POST['new_status'])) {
        error_log("Missing username or new_status for toggle_status");
        echo json_encode(['success' => false, 'error' => 'Missing technician username or new status.']);
        exit();
    }

    $username = trim($_POST['username']);
    $newStatus = trim($_POST['new_status']);
    error_log("Toggle status requested for username: $username to $newStatus");

    // Log the status change
    $logDescription = "Toggled status for technician $username to " . $newStatus . " by $firstName $lastName";
    $logType = "Staff $firstName $lastName";
    $sqlLog = "INSERT INTO tbl_logs (l_stamp, l_description, l_type) VALUES (NOW(), ?, ?)";
    $stmtLog = $conn->prepare($sqlLog);
    if ($stmtLog) {
    ...
    if (isset($_GET['action']) && $_GET['action'] === 'search') {
        ...
        $sql = "SELECT u.u_fname, u.u_lname, u.u_email, u.u_type, u.u_status, u.u_username,
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                // Default status for all technicians - can be toggled via frontend
                ...
                echo "<tr data-username='" . htmlspecialchars($row['u_username'], ENT_QUOTES, 'UTF-8') . "' data-status='" . $statusDisplay . "'>
                        ...
                        <td class='action-buttons'>
                            ...
                            <a class='assign-btn' href='#' onclick=\"handleAssignClick('" . htmlspecialchars($row['u_username'], ENT_QUOTES, 'UTF-8') . "', '" . $technicianName . "', '" . $statusDisplay . "')\" title='Assign Ticket'><i class='fas fa-ticket'></i></a>
                        </td>
                    </tr>";
            }
        } else {
    ...
    // Handle AJAX search request for tickets
...
<head>
    ...
    <link rel="stylesheet" href="AssignTechss.css">
                ...
                <tbody id="technicians-tbody">
                    ...
                    if ($resultTechnicians && $resultTechnicians->num_rows > 0) {
                        while ($row = $resultTechnicians->fetch_assoc()) {
                            // Default status for all technicians
                            $statusDisplay = 'Available';
                            $badgeColor = '#00b894';
                            $technicianName = htmlspecialchars($row['u_fname'] . ' ' . $row['u_lname'], ENT_QUOTES, 'UTF-8');
                            echo "<tr data-username='" . htmlspecialchars($row['u_username'], ENT_QUOTES, 'UTF-8') . "' data-status='" . $statusDisplay . "'>
...
<script>
// Global variable to track technician statuses
let technicianStatuses = {};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Initialize all technicians as Available
    initializeTechnicianStatuses();
    
    setInterval(searchTechnicians, 30000);
});

function initializeTechnicianStatuses() {
    // Get all technician rows and set initial status
    const rows = document.querySelectorAll('#technicians-tbody tr');
    rows.forEach(row => {
        const username = row.getAttribute('data-username');
        if (username && !technicianStatuses[username]) {
            technicianStatuses[username] = 'Available';
        }
    });
}
...
function searchTechnicians(page = 1) {
    ...
    .then(data => {
        if (data.success) {
            ...
            // Update statuses for newly loaded technicians
...
function updateTechnicianStatuses() {
    const rows = document.querySelectorAll('#technicians-tbody tr');
    rows.forEach(row => {
        const username = row.getAttribute('data-username');
        if (username) {
            // Use stored status or default to Available
            const currentStatus = technicianStatuses[username] || 'Available';
            
            // Update the badge display
            const badge = row.querySelector('.status-badge');
            if (badge) {
                badge.textContent = currentStatus;
                badge.style.backgroundColor = currentStatus === 'Available' ? '#00b894' : '#d63031';
            }
...
function toggleTechnicianStatus(username) {
    ...
    technicianStatuses[username] = newStatus;
    ...
    if (row) {
        ...
        if (badge) {
            ...
            badge.style.backgroundColor = newStatus === 'Available' ? '#00b894' : '#d63031';
        }
    ...
    fetch('AssignTech.php', {
        ...
        body: `action=toggle_status&username=${encodeURIComponent(username)}&new_status=${encodeURIComponent(newStatus)}`
    })
    .then(response => {
        if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
        return response.json();
    ...
    .then(data => {
        ...
        successAlert.textContent = `Status for technician ${username} updated to ${newStatus}.`;
    ...
    .catch(error => {
        ...
        // Even if server call fails, the local status is already updated
        const successAlert = document.createElement('div');
        successAlert.className = 'alert alert-success';
        successAlert.textContent = `Status for technician ${username} updated to ${newStatus}.`;
        alertContainer.appendChild(successAlert);
...
function handleAssignClick(technicianUsername, technicianName, status) {
    ...
    const currentStatus = technicianStatuses[technicianUsername] || 'Available';
    if (currentStatus === 'Unavailable') {
        const alertContainer = document.querySelector('.alert-container');
