...
if (isset($_GET['id'])) {
    ...
    $sql = "SELECT a_id, a_name, a_status, a_quantity, a_date FROM tbl_borrow_assets WHERE a_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $sqlUpdate = "UPDATE tbl_borrow_assets SET a_name = ?, a_status = ?, a_quantity = ?, a_date = ? WHERE a_id = ?";
    $stmtUpdate = $conn->prepare($sqlUpdate);
    $stmtUpdate->bind_param("ssisi", $assetName, $assetStatus, $assetQuantity, $assetDate, $assetId);
    
    if ($stmtUpdate->execute()) {
        echo "<script>alert('Asset updated successfully!'); window.location.href='staffD.php';</script>";
    } else {
